	...
	$("#lottery_form").validate({
		submitHandler:function(form){
			...
			if(flag){
				...
				var descr	=$("#descr").val();
				lottery["id"]  =parseInt(id,10);
				lottery["name"]=$.trim(name);
				lottery["rate"]=parseFloat(rate);
				lottery["status"]=parseInt(status,10);
				...
				$("#prizes_term tr:gt(0)").each(function(i,obj){
					...
					var id		=$(obj).find("input[type=hidden][name='prize.id']").val();
					...
					var status	=$(obj).find("select[name='status'] option:selected").val();
					if(prize==1){
						flag=true;
					}
				...
				if(temp==count){
					...
					if(flag==true){
						Loading("正在提交数据,请稍后...");
						$.ajax({
							url:_path+'/service/lottery/modify',
							type:'post',
